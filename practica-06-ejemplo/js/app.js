...
function renderizar(lista) {
  ...
  lista.forEach(item => {
    ...
    const listaFrases = document.createElement('ul');

    for (let i = 0; i < 5; i++) {
      const frase = document.createElement('li');

      if (!item.phrases || item.phrases.length === 0) {
        frase.textContent = 'No hay frases disponibles';
        listaFrases.appendChild(frase);
        break;
      } 
      else if (i >= item.phrases.length) {
        frase.textContent = 'No hay más frases disponibles';
        listaFrases.appendChild(frase);
        break;
      } 
      else {
        frase.textContent = item.phrases[i];
      }
    ...
    bloqueTexto.appendChild(listaFrases);
